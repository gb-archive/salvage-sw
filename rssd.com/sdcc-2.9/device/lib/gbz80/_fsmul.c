/* Floating point library in optimized assembly for gameboy cpu
...
1$:                     ;   loop{
    ...
    adc     d           ;       *rt++ += x[1];
    ld      (hl+),a
                        ;       // ripple carry to high order bytes
7$: jr      nc,2$       ;       while(carry bit && --kt > 0)
    dec     c           ;       
    jr      z,2$   
    ld      a,#0        ;            *rt++ += carry;
    adc     (hl)        ;        
...
2$: lda     hl,ku(sp)   ;       if(--ku == 0)
    ...
    lda     hl,rp(sp)   ;       ++rp; // move to next significant byte
    inc     (hl)
    jr      nz,6$
    inc     hl
    inc     (hl)

6$: lda     hl,up(sp)   ;       ++up; // move to next significant byte
    inc     (hl)
    jr      nz,1$
    inc     hl
    inc     (hl)
...
4$: lda     hl,kv(sp)   ;    if(--kv == 0)
    ...
    lda     hl,rp0(sp)  ;    ++rp0; // move to next significant byte
    inc     (hl)
    jr      nz,11$
    inc     hl
    inc     (hl)

11$: lda    hl,vp(sp)   ;    ++vp; // move to next significant byte
    inc     (hl)
    jr      nz,0$
    inc     hl
    inc     (hl)
...
5$: ;; leave result in normalized form with space for lsb of exponent
...
3$: lda     hl,sign(sp) ; retrieve the new sign
    ld      a,(hl+)
...
ERANGE 
Result too large. An argument to a math function is too large, 
resulting in partial or total loss of significance in the result. 
This error can also occur in other functions when an argument is 
larger than expected.  Under/Overflow
EDOM 
Math argument. NaN
*/
...
;--------------------------------------------------------
	.area _CODE
;_fsadd.c:6: float __fsadd (float u, float v)
;	---------------------------------
; Function __fsadd
; ---------------------------------
___fsadd:
	
;_fsadd.c:287: __endasm;
	...
	shift0:
		   ...
		   jp pack
	0$:
	bit	7,c ; if exp(v) - exp(u) > 0
		   jr nz,1$
		   ld a,c
		   lda hl,u0(sp) ; shift u to the right and add
		   jr shift
	1$:
	ld	a,c ; if exp(v) - exp(u) > -25
		   sub #-25
		   bit 7,a
		   jr nz,$2
		   ld a,c

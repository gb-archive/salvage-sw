...
genFunction (iCode * ic)
{
  ...
  */
  ...
  if(IFFUNC_BANKED (sym->type)){
      int bank_number;
      /* emit symbol for bank number */
      //emit2(".globl b%s=_%s", sym->rname, options.code_seg);
      const char * x = options.code_seg;
      sscanf(options.code_seg, "CODE_%i", &bank_number);
      emit2("b%s = %i", sym->rname, bank_number);
  }

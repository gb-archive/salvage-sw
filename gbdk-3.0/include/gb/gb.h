...
/** Halt until the next interrupt occurs.  Used for
    saving power.
 */
void
halt() NONBANKED;
...
/** Enables unmasked interrupts
    decrements the pending interrupt counter.  When this counter
    goes to zero, interrupts are re-enabled.
    @see disable_interrupts
*/
void
enable_interrupts(void) NONBANKED;

/** Disables interrupts and increments the pending interrupt counter.

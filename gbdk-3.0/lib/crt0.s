	...
	.area	_HEADER (ABS)
	;; locaton for Standard header for the GB
	.org	0x00
	.area	_CODE
	.area	_BASE
	;; Code
...
.int:
	...
	PUSH	BC
	PUSH	DE
	; increment the depth of the interrupt counter
    LD  A, (__ei_depth)
    INC A
    LD  (__ei_depth), A
1$:
...
; 	LD	(.mode),A	; Clearing (.mode) is performed when clearing RAM
	...
	ld  a, #1               ; initialize switched bank to 1 before
	ld	(__current_bank),a  ; before enabling interrupts
	ld	(.MBC1_ROM_PAGE),a	; Perform the switch
	xor a,a                 ; initialize interrupt depth to 0
	ld  (__ei_depth), a
...
__current_bank:
	.ds	0x01		; Current MBC1 style bank. - initlialize as 1
__ei_depth:  
    .ds  0x01   ; depth of int enable
...
banked_ret::
    ; note - preserve DE and HL as these are used to pass
    ; back return values
	call    _disable_interrupts
